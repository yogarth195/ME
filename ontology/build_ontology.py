...
def build_graph() -> nx.DiGraph:
    ...
    # ── O*NET additions (auto-generated by update_from_onet.py) ──────────────
    try:
        from onet_additions import ONET_CHILD_OF
        added = 0
        for skill, parent in ONET_CHILD_OF:
            if not G.has_node(skill):
                G.add_node(skill)
                added += 1
            if not G.has_node(parent):
                G.add_node(parent)
            G.add_edge(parent, skill, relation="CHILD_OF")
        print(f"  O*NET additions: {added} new nodes, {len(ONET_CHILD_OF)} edges")
    except ImportError:
        pass
